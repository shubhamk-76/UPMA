...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Before transform
        # print(f"[Before Transform] mask unique: {np.unique(mask)}")

        # print(mask)
        if image is None or mask is None:
            print(f"[WARN] Skipping unreadable sample:\n    Image: {imagepath}\n    Mask : {maskpath}")
        # Try next sample (wraps around to the beginning if at the end)
            return self.__getitem__((idx + 1) % len(self.samples))
        
        image = image.astype(np.float32)[:, :, ::-1]
        mask = mask.astype(np.float32)[:, :, ::-1]
        # print(f'mask-shape-is{mask.shape}')f0
        # mask = mask.astype(np.int64)
        # print(f"Image{image.dtype},mask{mask.dtype}")
        
        H, W, C             = mask.shape
        if self.cfg.mode == 'train':
            image, mask         = self.transform(image, mask)
            # print(f"[After Transform] mask unique: {torch.unique(mask)}")

            # print(f"after transform:{mask.shape}")
            # mask[mask == 0.] = 255.
            # mask[mask == 2.] = 0.
            mask = (mask>0).long()
        else:
...
if __name__=='__main__':
    ...
    plt.ion()
    cfg  = Config(mode='test', datapath='/mnt/DATA/CS24S001/PCOD/Point supervised camouflaged object detection/CodDataset')
    data = Data(cfg)
    loader = DataLoader(data, batch_size=2, shuffle=True, num_workers=8)
    prefetcher = DataPrefetcher(loader)
    batch_idx = -1
    image, mask = prefetcher.next()
    image = image[0].permute(1,2,0).cpu().numpy()*cfg.std + cfg.mean
    mask  = mask[0].cpu().numpy()
    print("mask unique after transform:", torch.unique(mask))
