...
def validate(model, val_loader):
    model.train(False) #This is similar to model.eval()
    avg_mae = 0.0
    cnt = 0
    skipped = 0 
    ...
    with torch.no_grad():
        for image, mask, shape, name in val_loader:
            try:
                ...
                out = F.interpolate(out, size=tuple(shape), mode='bilinear', align_corners=False)

                pred = torch.sigmoid(out[0, 0])
                pred = (pred - pred.min()) / (pred.max() - pred.min() + 1e-8)

                # Ensure pred is 4D for interpolation
                if pred.ndim == 2:
                    pred = pred.unsqueeze(0).unsqueeze(0)

                pred = F.interpolate(pred, size=tuple(mask[0].shape[-2:]), mode='bilinear', align_corners=False)
                pred = pred.squeeze()

                # Process mask safely
                mask_tensor = mask[0]
                if mask_tensor.ndim == 0 and mask_tensor.shape[0] == 1:
                    mask_tensor = mask_tensor.squeeze(0)

                if pred.shape != mask_tensor.shape:
                    print(f"[Skipped] Shape mismatch in {name}: pred {pred.shape}, mask {mask_tensor.shape}")
                    skipped += 1
                    continue

                avg_mae += torch.abs(pred - mask_tensor).mean().item()
                cnt += len(image)

            except Exception as e:
                print(f"[Skipped] Error in {name}: {e}")
                skipped += 1
                continue

    model.train(True)
    print(f"[Validation] Skipped {skipped} problematic samples.")
...
MAX_LR = 1e-2
total_epoch = 60
EXP_NAME = 'Exp-11' # change it in main
root = './CodDataset'
...
def train(Dataset, Network, cfg, train_loss, start_from = 0):
    ...
    val_cfg = [Dataset.Config(datapath=f'{root}', mode='train')]
    ...
    val_loaders = [DataLoader(v, batch_size=2, shuffle=False, num_workers=4) for v in val_data] 
    min_mae = 1.0
    best_epoch = 0
    ## network
    #by_chf
    ...
    ## log
    print(f'cfg.save path is{cfg.savepath}')
    ...
    for epoch in range(start_from, cfg.epoch):
        ...
        import gc 
        gc.collect() # Clearing cuda cache
        torch.cuda.empty_cache()
...
if __name__=='__main__':
    ...
    EXP_NAME = f'Exp-11'
    cfg = dataset.Config(datapath=f'{root}', savepath=f'./out/{EXP_NAME}/', mode='train', batch=2, lr=1e-3, momen=0.9, decay=5e-4, epoch=total_epoch, label_dir = 'Label')
    ## ---------Representation Optimizer flags-----------##
    cfg.use_rep_opt = True
    cfg.lambda_c = 1.0
